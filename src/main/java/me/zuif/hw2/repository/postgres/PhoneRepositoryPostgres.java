...
@Singleton
public class PhoneRepositoryPostgres implements ProductRepository<Phone> {
    private static final Connection CONNECTION = JDBCConfig.getConnection();

    private static PhoneRepositoryPostgres instance;

    @Autowired
    public PhoneRepositoryPostgres() {
    }

    public static PhoneRepositoryPostgres getInstance() {
        if (instance == null) {
            instance = new PhoneRepositoryPostgres();
        }
        return instance;
    }
