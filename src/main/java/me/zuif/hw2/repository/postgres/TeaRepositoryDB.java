...
@Singleton
public class TeaRepositoryDB implements ProductRepository<Tea> {
    private static final Connection CONNECTION = JDBCConfig.getConnection();

    private static TeaRepositoryDB instance;

    @Autowired
    public TeaRepositoryDB() {

    }

    public static TeaRepositoryDB getInstance() {
        if (instance == null) {
            instance = new TeaRepositoryDB();
        }
        return instance;
    }
